...
| 크롤링        | Playwright     | Puppeteer, **Apify** | lib/adapters/crawler.ts |
| 스크래핑 마켓 | (Phase 2 검토) | **Apify**            | lib/adapters/apify.ts   |
| PDF           | ReportLab      | Puppeteer PDF        | lib/adapters/pdf.ts     |

> **Apify 활용 메모 (2026-03-13):** Apify Actor 마켓플레이스를 Epic 3(크롤링)과 Epic 6(경쟁사 분석)에서 활용 가능.
> 경쟁사 자동 탐색(Google SERP Actor), 백링크/브랜드 언급 추적에 유용.
> 현재 Epic 1 진행 중이므로 구현은 Epic 3/6 단계에서 검토. `lib/adapters/apify.ts`로 어댑터 패턴 적용 예정.
