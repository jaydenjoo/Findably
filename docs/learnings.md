...
### 2026-04-27 dairect.kr 14개 오진 — 부분 패치 범위 협소 + AI 가드레일 부재 (2026-04-13의 다른 필드 재발)

- **증상**: dairect.kr Findably 진단 PDF에서 32개 항목 중 14개 오진. "사이트 0KB 로딩 실패 (치명적 오류)", "SSL 인증서 완전 누락", "Core Web Vitals 완전 실패 (LCP 29초)", "H1 태그 누락", "Title 부재", "http://로 접속" 등 단정적 표현. 그러나 실제 사이트(curl 검증)는 모든 메타 정상: title/description/h1/canonical/og 10개/twitter card 5개/JSON-LD 1개/HSTS 2년. 13일 전(2026-04-13) 동일 사이트 진단에서는 정상이었음
- **원인**: 3겹 복합. (1) **크롤러**: Firecrawl/n8n이 dairect.kr head를 부분 누락 — title/description만 metadata에 채워 보내고 h1/canonical/og/json-ld는 빈값, page_size_bytes=0/load_time_ms=0(필드 자체 미수집), ssl=null. (2) **폴백 부재**: 2026-04-13 패치의 parseHeadFromHtml 폴백이 canonical/og/jsonLd만 보강하고 **title/description/h1은 폴백 안 함**. enrichCrawlData의 fetchHeadMetadata도 title/description/h1을 머지하지 않음. UA(`FindablyBot/1.0`)가 일부 사이트에서 차단될 가능성도 있음. (3) **AI 환각**: 5개 에이전트 + CMO가 page_size=0/ssl=null을 그대로 받고 가드레일 부재 → "0KB 로딩 실패", "전기 끊어진 상태", "http://로 접속", "보안 자물쇠 없음" 등 단정 환각 생성
- **해결**: 6개 파일 수정 (커밋 `0d39f9b`). (a) head-metadata.ts에 title/description/h1 추출 + UA 2단 fallback (FindablyBot 1차 → 차단 시 일반 브라우저 UA 재시도) (b) parse-crawl-v2.ts/enrich-crawl-data.ts 폴백 범위에 title/description/h1 포함 (c) guards.ts에 hasValidPageSize/hasValidLoadTime 추가 — page_size=0이면 룰 isEvaluable false로 skip (d) performance.ts perf-02에 LCP>20초 STALE 캐시 가드 메시지 (e) run-diagnosis-paid.ts buildCrawlSummary 시작에 글로벌 가드레일 + page_size/SSL/LCP 측정 불가 명시
- **규칙**: **부분 패치는 같은 메커니즘이 다른 필드에서 재발한다**. 폴백 시스템은 한 번에 의미 있는 영역(head 메타: title/description/h1/canonical/og/json-ld/links)을 일관되게 다루어야 하며, 일부만 처리하면 다음 사이트에서 빠진 필드가 누락되어 진단 정확도를 다시 깎는다. **AI 에이전트에 데이터 가드레일 필수**: 측정 불가 데이터(0/null)를 그대로 프롬프트에 박지 말고 "측정 불가 ⚠️ — 단정 금지" 같은 메타 가드를 붙여야 환각 차단. **외부 크롤링 도구 응답은 시간이 흐르며 변할 수 있다**: 같은 사이트 동일 코드 진단 결과가 13일 만에 정반대로 나올 수 있음 — 카나리 자가진단에 외부 사이트도 포함하고, 신호 누락 패턴이 보이면 폴백 범위 전수 점검. **디버깅 순서**: 다른 AI/사람이 "오진"이라 보고하면 (1) 실제 사이트 curl 직접 확인 (2) Supabase crawl_data 실측값 비교 (3) 4계층 의심(크롤러 → 파서 → 엔진 → AI 에이전트) 후 단일 layer 단정 금지

### 2026-04-27 dairect.kr 14개 → 5개 새 오진 (Fix 6) — 부분 패치 패턴 3차 재발 + AI 중복 + 폐기 지표

- **증상**: Fix 1~5 적용 후 dairect.kr 재진단에서 14개 → 5개 새 오진. h1/canonical/og는 정상화됐으나 (a) H2/H3/H4 미감지(실제 12+10개), (b) 내부 링크 1개(실제 29+개), (c) FID 폐기 지표 사용 중(2024-03부터 Google INP로 대체), (d) AI 5 에이전트 중복 보고(LCP·SSL 각각 두 번씩 PDF 노출), (e) LCP 26초 측정도구 의심
- **원인**: (1) parseHeadFromHtml/HeadMetadata가 h1만 추출 — Fix 1A 시 h1만 반영한 부분 패치 → h2/h3/h4 누락 시 폴백 못 함. (2) 링크 폴백 조건 `internalLinks === 0`이라 Firecrawl이 1 반환하면 폴백 안 됨 (실제 사이트 22~29개를 1개로 저장). (3) Google이 2024-03부터 Core Web Vitals를 FID → INP로 변경했는데 perf-03 룰이 여전히 FID(`max-potential-fid`) 기준. (4) 5 에이전트 결과 통합(`agentResults.flatMap`) 시 dedup 부재 → CMO도 통합 안 함 → 같은 항목이 다른 표현으로 PDF에 중복 노출
- **해결**: 5개 파일 수정 (Fix 6A~6D). (a) head-metadata.ts에 `extractHeadings(level)` 클로저 헬퍼 + 인터페이스에 h2/h3/h4 추가, parse-crawl-v2.ts/enrich-crawl-data.ts 폴백·머지 로직에 모두 포함 (b) 링크 폴백 조건을 `htmlExtracted > internalLinks`(Math.max 패턴)로 강화 — 1이든 5든 HTML 추출이 더 크면 우선 (c) perf-03을 INP 기반으로 전환 (CrUX `inp_ms` 사용, 없으면 skip), buildCrawlSummary에서 FID 라인을 "폐기 지표 사용 금지" 가드 메시지로 변경 (d) run-diagnosis-paid.ts에 `dedupeInsights<T>` helper 추가 — (category, normalized title) 키로 중복 차단, aggregateResults + executeCmoAgent 양쪽 적용
- **규칙**: **부분 패치는 같은 메커니즘이 다른 필드/지표에서 재발한다 — 3차 재발 확인** (2026-04-13 canonical/Schema/OG → 2026-04-27 1차 title/description/h1 → 2026-04-27 2차 h2/h3/h4/links/FID). 폴백 시스템(parseHeadFromHtml)은 처음부터 **영역(head 메타) 전체를 일관되게 다뤄야** 하며, "이 필드는 보강, 저 필드는 안 함" 같은 부분 적용은 다음 사이트에서 다른 필드 누락으로 재진단 정확도를 깎음. **외부 표준 변경 추적 필요**: Google/Apple 메이저 업체 SEO/CWV 변경(FID→INP, Lighthouse 버전업 등)은 3-6개월 단위 점검 — 학습 데이터 cutoff 이후 변경된 것이 룰에 박혀있을 수 있음. **AI 에이전트 dedup 필수**: 같은 데이터를 5개 에이전트가 분석하면 중복 결론 발생 — aggregateResults/CMO 단계에서 정규화 키 기반 dedup 코드 추가가 LLM 가드보다 안전. **검증 우선순위**: dairect.kr/findably.kr 같은 외부 사이트를 카나리 회귀에 포함하면 부분 패치 누락을 다음 PR에서 잡을 수 있음 — Fix 6F로 별도 Task 분리
